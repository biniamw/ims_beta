            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <div class="card-header border-bottom">
                        <div style="width:19%;">
                            <h3 class="card-title">Stock Beginning</h3>
                                ...
                                <table id="laravel-datatable-crud" class="display table-bordered table-striped table-hover dt-responsive" style="width: 100%">
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="width: 0%;">#</th>
                                            <th style="width: 20%;">Beginning Doc. No.</th>
        ...
        <div class="modal-content">
            ...
            <form id="bgInfoform">
                ...
                <div class="modal-footer">
                    ...
                    @endcan
                    <button id="closebuttonb" type="button" class="btn btn-danger" data-dismiss="modal" onclick="closeInfoModal()">Close</button>
                </div>
            </form>
        </div>
    </div>
  ...
  <!-- End Info -->
    ...
    <div class="modal fade text-left" id="postedInfoModal" data-keyboard="false" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="myModalLabel33" aria-hidden="true" style="overflow-y: scroll;">
        <div class="modal-dialog modal-xl" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="myModalLabel33">Beginning Info</h4>
                    <div>
                ...
                <form id="postedinfoform">
                    ...
                    <div class="modal-footer">
                        <button id="closebuttonc" type="button" class="btn btn-danger" onclick="closeInfoModal()" data-dismiss="modal">Close</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="modal-dialog modal-xl" role="document">
            <div class="modal-content">
                ...
                <form id="adjustmentform">
                                    ...
                                    <div class="table-responsive">
                                        ...
                                        <table class="table table-bordered table-striped table-hover dt-responsive table mb-0">
                                            <tr>
                                                ...
                                                <th><label strong style="font-size: 14px;">Fiscal Year</label></th>
                                            ...
                                            <tr>
                                                ...
                                                <td><label id="adjinfosfiscalyear" strong style="font-size: 14px;"></label></td>
                    ...
                    <div class="modal-footer">
                        <button id="closebuttonk" type="button" class="btn btn-danger" onclick="closeInfoModal()" data-dismiss="modal">Close</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    $(document).ready( function () {
        ...
        $('#laravel-datatable-crud').DataTable(
        {
            ...
            columns: [
                ...
                { data: 'EndingDocumentNo', name: 'EndingDocumentNo'},
   ...
   //Datatable read property ends
    ...
    $('body').on('click', '.addadjbutton', function () {
        ...
        $.get("/getbgnumber"  , function (data) {
            ...
            $('#beginingtitle').text("Begining Form (Fiscal Year: "+data.FiscalYear+")");
        });
           ...
           url:'/savebg',
            ...
            success:function(data) 
            {
                ...
                if(data.duplicateerror)
                {
                    ...
                    $('#savebutton').text('Save');
                    $('#savebutton').prop( "disabled", false );
                    toastrMessage('error',"Store already exist","Error");   
                }
                if(data.success) {
                    $('#savebutton').text('Save');
           ...
           url:'/startCount',
            ...
            success:function(data) 
            {
                if(data.success) 
                {
                    ...
                    $.get("/showBgData" +'/' + recordId , function (data) 
                    {     
                        var dc=data;
                        ...
                        {  
                            ...
                            $('#infodocennum').text(data.bgHeader[i].EndingDocumentNo);
    ...
    $('body').on('click', '.editBegining', function () 
    {
        ...
        else
        {
            ...
            $.get("/beginingedit" +'/' + recIdVar , function (data) { 
                $('#store').select2('destroy');
                $('#store').val(data.begining.StoreId).select2();
                $('#beginingtitle').text("Begining Form (Fiscal Year: "+data.fyr+")");
            });
            $('#inlineForm').modal('show');
            $('#savebutton').prop( "disabled", false );
            $('#savebutton').text('Save');
    ...
    $(document).on('click', '.infoCounting', function()
    {
        ...
        {     
            ...
            {  
                ...
                $('#infosdocennum').text(data.bgHeader[i].EndingDocumentNo);
        ...
        ajax: {
            ...
            url: '/showDetailBgPosted/'+recordId,
            type: 'DELETE',
            },
        columns: [
            { data: 'id', name: 'id','visible': false },
    ...
    $(document).on('click', '.adjustmentBegining', function()
    {
        ...
        {     
            ...
            {  
                ...
                st=data.bgHeader[i].Status;
                var fyear=data.bgHeader[i].FiscalYear;
                var nfyear=parseFloat(fyear)+1;
                var fyearrange=fyear+"/07/08-"+nfyear+"/07/07";
                $('#adjinfosfiscalyear').text(fyearrange);
    ...
    function closeInput(elm) {
        ...
        if(value=="")
        {
            $.get("/updateQ/"+ids+'/'+'-1'  , function (data) {});
        }
        else if(val<0)
        {
            $("#quantityId").val("");
        ...
        {    
            $.get("/updateQ/"+ids+'/'+value  , function (data) {});
            var iTable = $('#doneinfodetail').dataTable(); 
            iTable.fnDraw(false);            
        }
    }
    ...
    function uccloseInput(elm) {
        ...
        if(value=="")
        {
            ...
            var ids=$("#editRowDatai").val();
            var dval="NULL";
            $.get("/updateUp/"+ids+'/'+'-1'  , function (data) {});
        }
        else if(value!="")
        {
            var ids=$("#editRowDatai").val();
            $.get("/updateUp/"+ids+'/'+value  , function (data) {});
        }
    }
        ...
        $.ajax({
           url:'/editPostedSingleItem',
            ...
            success:function(data) {
                ...
                if(data.success) {
                    $('#updateAdjustmentItem').text('Save');
                    toastrMessage('success',"Posted Item Adjusted Successfully","Success");
                    $("#newItemModal").modal('hide');
                    $("#adjNewItemForm")[0].reset();
                    closeSaveNewModal();
                    var oTable = $('#adjustmentdetail').dataTable(); 
                    oTable.fnDraw(false);
                    $('#adjustmentdetail').DataTable().ajax.reload();
                    var recordId= $("#headeridi").val();
                    $.get("/showBgData" +'/' + recordId , function (data) 
                    {     
                        var dc=data;
           ...
           url:'/deletePostedItem',
            ...
            success:function(data) {
                ...
                if(data.success) {
                    ...
                    toastrMessage('success',"Posted Item Removed Successfully","Success");
                    $("#postedRemoveModal").modal('hide');
                    closeSaveNewModal();
                    var oTable = $('#adjustmentdetail').dataTable(); 
                    oTable.fnDraw(false);
    ...
    { 
        ...
        $.get("/showBgItemInfo" +'/' + sid+'/'+storeid+'/'+headerid , function (data) 
        {    
            if(data.valerror)
            {
            ...
            { 
                ...
                {  
                    ...
                    {  
                        ...
                        $('#uoms').val(itemUOMVar);
                        $('#MinSelling').val((parseFloat(itemWholeseller)/1.15).toFixed(2));
                    } 
                } 
            }         
        });
...
$('body').on('click', '.editPosted', function () 
{
    ...
    $('#uoms').val(uomname);
    $('#MinSelling').val(parseFloat(minimumsale/1.15).toFixed(2));
    $('#updateAdjustmentItem').text('Save');
    $('#updateAdjustmentItem').prop( "disabled", false );
    $("#newItemModal").modal('show');
    $.get("/editPostedItem" +'/' + recIdVar , function (data) 
    {
...
$('body').on('click', '.addSerialNumber', function () 
{
    ...
    $.get("/showSerialNumberBg" +'/' + sid , function (data) 
    {
        ...
        else
        {
            ...
            $('#laravel-datatable-crud-sn').DataTable({
                ...
                "<'row'<'col-sm-12 col-md-3'l><'col-sm-12 col-md-5'i><'col-sm-12 col-md-3'p>>",
                ajax: {
                    url: '/showSerialNmBg/'+headerid+'/'+itemid+'/'+trtype,
                    type: 'GET',
                    },
                columns: [
                    { data: 'id', name: 'id', 'visible': false },
    ...
    $("#downloatoexcel").click(function(){
        ...
        name: "Worksheet Name",
        filename: "BeginingDetailTable", //do not include extension
    ...
    $("#downloatoexcels").click(function(){
        ...
        name: "Worksheet Name",
        filename: "BeginningPostedItem", //do not include extension
        fileext: ".xls" // file extension
        });
    });
