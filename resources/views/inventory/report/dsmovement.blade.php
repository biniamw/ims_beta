...
@section('content')
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header border-bottom fit-content">
                                <h3 class="card-title form_title">Direct Stock Item Movement Report</h3>
                            </div>
                            <form id="movemntform">
                                @csrf  
    ...
    <script type="text/javascript">
        ...
        function fetchMovementDataFn(){
            ...
            table = $("#movementtable").DataTable({
                ...
                ajax: {
                    ...
                    url: '/dsmovementshow/' + from_date + '/' + to_date + '/' + str_id + '/' + f_year + '/' + trn_type,
                    type: 'POST',
                    data:{
                        itemnames: $('#items').val(),
                    },
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        width:"10%",
                        "render": function ( data, type, row, meta ) {
                            return `<a style="text-decoration:underline;color:blue;" onclick=refNumber("${row.TransactionsType}","${row.HeaderId}","${data}","${row.StockIn}","${row.StockOut}")>${data}</a>`;
                        }     
                    },
                    {
                        data: "Date",
        ...
        $("#downloatoexcel").click(function () {
            ...
            var worksheet = workbook.addWorksheet("DS_Item_Movement_Report");

            let mergeCells = [];
            let maxColumnWidths = {}; // Store max column widths

            let titleRow = worksheet.addRow(["Direct Stock Item Movement Report"]);
            ...
            workbook.xlsx.writeBuffer().then((data) => {
                ...
                saveAs(blob,`DS_Item_Movement_from_${fromdate}_to_${todate}.xlsx`);
            });
        });
        ...
        $("#downloadtopdf").click(function () {
            try {
                ...
                doc.text("Direct Stock Item Movement Report", doc.internal.pageSize.width / 2, 35, { align: "center" });
                ...
                // Save PDF
                const fileName = `DS_Item_Movement_Report_${fromdate}_to_${todate}.pdf`;
                doc.save(fileName);
        ...
        function refNumber(transactiontype,transactionid,referencenum,stock_in,stock_out){
            if(transactiontype == "Begining"){
                var link = "/dsbgp/" + transactionid;
                window.open(link, 'Begining', 'width=1200,height=800,scrollbars=yes');
            }
            if(parseFloat(stock_in) > 0 && (transactiontype == "Purchase" || transactiontype == "Transfer" || transactiontype == "Transfer-In" || transactiontype == "Others")){
                var link = "/dshi/" + transactionid;
                window.open(link, 'Stock-In', 'width=1200,height=800,scrollbars=yes');
            }
            if(parseFloat(stock_out) > 0 && (transactiontype == "Sales" || transactiontype == "Internal" || transactiontype == "Others")){
